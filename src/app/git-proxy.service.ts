...
export class GitProxyService {
  ...
  getTree(owner: string, repo: string, sha: string, recursive?: number, access_token?: string): Promise<any> {
    let url = apiBasePath + '/v5/repos/' + owner + '/' + repo + '/git/gitee/trees/' + sha;
    let params = {};
    if (access_token) {
      params = recursive
  ...
  getLabels(owner: string, repo: string): Promise<any[]> {
    let url = apiBasePath + '/v5/repos/' + owner + '/' + repo + '/labels';
    return this.http
      .get<any[]>(url)
      .toPromise();
  }
  ...
  getCommentsOfIssue(owner: string, repo: string, number: string, page = 1, perPage = 20, accessToken?: string): Promise<any[]> {
    let url = apiBasePath + '/v5/repos/' + owner + '/' + repo + '/issues/' + number + '/comments';
    let params = accessToken
